...
class FhcPage extends StatefulWidget {
  final int categoryId = 1; // กำหนดเป็น 0 สำหรับหน้า FHC (ตู้น้ำดับเพลิง)
...
class _FhcPageState extends State<FhcPage> {
  ...
  final String apiUrl = 'https://api.jaroonrat.com/safetyaudit/api/assetlist/2';
  ...
  Future<void> fetchFire() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          fhcList = List<Map<String, dynamic>>.from(data['asset'] ?? []);
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Widget _summaryCard(int total, int filtered) {
    const icon = Icons.fire_hydrant_alt;
    const color =Colors.deepOrangeAccent;
    const categoryName = 'ตู้น้ำดับเพลิง';
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Column(
            children: [
              const Icon(Icons.fire_hydrant_alt, size: 46, color: Colors.deepOrangeAccent),
            ...
            children: [
              ...
              _actionButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const AuditFhcDetailPage(auditedAssetIds: [])),
                  );
                },
                icon: Icons.history,
                label: 'ประวัติ',
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> filteredList = fhcList.where((item) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('ตู้น้ำดับเพลิงทั้งหมด', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: Colors.red))
          : Column(
              children: [
                _summaryCard(fhcList.length, filteredList.length),
                const SizedBox(height: 10),
                _searchBar(fhcList), // 📍 เรียกใช้ SearchBar ใหม่ที่นี่
                Expanded(
