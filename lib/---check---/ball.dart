...
class _BallPageState extends State<BallPage> {
  ...
  Future<void> fetchFire() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ballList = List<Map<String, dynamic>>.from(data['asset'] ?? []);
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Widget _summaryCard(int total, int filtered) {
    ...
    const color =Color.fromARGB(255, 5, 47, 233);
    const categoryName = 'ลูกบอลดับเพลิง';
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Column(
            children: [
              const Icon(Icons.sports_baseball, size: 46, color: Color.fromRGBO(5, 47, 233, 1)),
            ...
            children: [
              ...
              _actionButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const AuditBallDetailPage(auditedAssetIds: [])),
                  );
                },
                icon: Icons.history,
                label: 'ประวัติ',
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> filteredList = ballList.where((item) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('ลูกบอลดับเพลิงทั้งหมด', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: Colors.red))
          : Column(
              children: [
                _summaryCard(ballList.length, filteredList.length),
                const SizedBox(height: 10),
                _searchBar(ballList), // 📍 เรียกใช้ SearchBar ใหม่ที่นี่
                Expanded(
